...
namespace YQTrack.Backend.OrderCompleteService.V2.Warn
{
    /// <summary>
    /// Seller 购买了查询数的。查询数不足10%的提醒
    /// </summary>
    public class SellerTrackNumAutoWarnWorkService
    {
        ...
        private DataRouteModel _druModel;
        ...
        public SellerTrackNumAutoWarnWorkService(DataRouteModel drModel)
        {
            _druModel = drModel;

            _orderAutoWarnBll = FactoryContainer.Create<IOrderAutoWarnBLL>();
            _orderAutoWarnBll.SetDataRoute(drModel);

            _userBLL = FactoryContainer.Create<IUserBLL>();
            _userBLL.SetDataRoute(drModel);
